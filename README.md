...
- **`control_node.py`**: 4절에서 설명한 FSM 전체. crazyswarm2의 액션 서버가 존재하지 않는다는 점(전부 fire-and-forget 서비스)을 고려해, `crazyflie_py`의 `Crazyswarm` 래퍼를 쓰지 않고 `/cfN/takeoff`,`/cfN/land`,`/cfN/go_to` 서비스 클라이언트를 직접 만들어 쓴다. 블로킹 sleep이 전혀 없는 10Hz 타이머 기반 논블로킹 FSM이라 `/detections` 구독을 비행 중에도 계속 처리한다.
  - 주요 파라미터: `mission_map_path`(필수), `cruise_speed`(기본 0.3 m/s), `min_leg_duration`(1.5s), `leg_settle_margin`(0.5s), `arrival_radius`(0.1m — 실측 위치가 이 안에 들어오면 "도착"으로 승격), `takeoff_duration`(2.0s), `takeoff_settle_time`(2.5s), `land_duration`(2.5s), `land_settle_time`(3.0s), `start_immediately`(false — GCS 버튼 없이 즉시 시작하고 싶을 때 true), `dead_zone_margin`(0.15m — 아래 `zone_split.py` 설명 참고).
  ...
  - **`/mission/progress`는 "명령을 보낸" 인덱스가 아니라 "실제로 도착 확인된" 인덱스**: `DroneHandle`이 `arrived_index`(방금 도착 완료한 waypoint)와 `pending_index`/`pending_target_xy`(현재 비행 중인 목표)를 분리해서 들고 있는데, `arrived_index` 승격은 **`leg_deadline`(예상 소요시간) 타이머가 아니라 `/states` 실측 위치가 `pending_target_xy`의 `arrival_radius`(기본 0.1m) 안에 들어왔을 때** 이루어진다(`_step_covering()`). `leg_deadline`은 텔레메트리가 끊겼을 때 영원히 멈추지 않게 하는 fallback 타임아웃일 뿐이다. 처음엔 타이머만으로 승격시켰는데, 실제 비행이 추정 소요시간보다 조금이라도 더 걸리면 GCS가 드론이 도착하기도 전에 셀을 칠하고(그 다음 실제 도착, 그 다음에야 `/detections`가 뜨는 순서로 어긋나 보임) — 실측 위치 기반으로 바꿔서 "칠하기/드론 도착/마커 감지"가 실제로는 거의 동시에 일어나도록 맞췄다. 드론마다 독립적으로 도착 확인 즉시 다음 구간으로 넘어가며(다른 드론을 기다리지 않음), 전부 `done`이 되면 `RETURN_HOME`으로 전환한다.
